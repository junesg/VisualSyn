...
params = struct('numvis', 2, ...
                ...
                'l2reg', 0.01, ...
                'gradcheck', 1);
weights = init_net_var(params,.1);

numdata = 10;
stdinit = 1;
X1 = stdinit*randn(params.numvis, numdata);
X2 = stdinit*randn(params.numvis, numdata);
X3 = stdinit*randn(params.numvis, numdata);
Y = false(numdata,1);
...
function [ cost, grad ] = wrap_grad(net, theta, X1, X2, X3, Y, pars)
    ...
    weights.id_comp = net.id_comp;
    [ grad, cost ] = grad_var(weights, pars, X1, X2, X3, Y);
    grad = roll(grad);
end
...
function [ weights ] = unroll(theta,pars)
    ...
    weights.hid_var_to_var = reshape(theta(idx:(idx+(pars.numvar*pars.numhid_var)-1)), ...
                      [pars.numvar,pars.numhid_var]);
    idx = idx + pars.numvar*pars.numhid_var;
    ...
    weights.id_pred = reshape(theta(idx:(idx+(pars.numid*pars.numid)-1)), ...
