function batch_general2e(fhandle, fname_prefix, fpars, varargin)
%BATCH_GENERAL2E    a modification of batch_general written by Prof. Lee
%   INPUT: batch_general2e(fhandle, fname_prefix, fpars, varargin)
...
%   EXAMPLE USAGE
%   
%  batch_general2e(@demo_pmrbm, 'jan04exp', ...
    ...
    function batch_recursion(filename, fpars, i_fpar)
        ...
        for val = vallist(randperm(length(vallist))) %vallist
            ...
            if iscell(val) % the code supports empty argument
                ...
                if isnumeric(val)
                    filename2 = sprintf('%s_%s%g', filename, valname, val);
                elseif ischar(val)
                    filename2 = sprintf('%s_%s%s', filename, valname, ...
                                        val);
                else islogical(val)
                    filename2 = sprintf('%s_%s%d', filename, valname, ...
                                        val);
                end
                theVal = val;
            end
            if pars.useStruct
                ...
                fpars_parsed{i_fpar} = theVal;
            end

            
            
            if length(fpars) > 2
                batch_recursion(filename2, fpars(3:end), i_fpar + 1);
            else                
